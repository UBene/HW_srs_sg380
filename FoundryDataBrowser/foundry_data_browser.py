...
try:
    from FoundryDataBrowser.viewers.h5_tree import H5TreeSearchView, H5TreeView
...
# try:
#     from FoundryDataBrowser.viewers.gauss2d_fit_img import (
#         Gauss2DFit_FiberAPD_View,
#         Gauss2DFitAPD_MCL_2dSlowScanView,
#         Gauss2DFitImgView,
#     )
...
# try:
#     from FoundryDataBrowser.viewers.apd_confocal_npz import (
#         ApdConfocal3dNPZView,
#         ApdConfocalNPZView,
#     )
...
try:
    from FoundryDataBrowser.viewers.picoquant_histogram_h5 import (
        PicoquantHistogramH5View,
    )
...
# try:
#     from FoundryDataBrowser.viewers.trpl_t_x_lifetime import (
#         TRPL_t_x_lifetime_fiber_scan_View,
#         TRPL_t_x_lifetime_H5_View,
#         TRPL_t_x_lifetime_NPZView,
#     )
...
# try:
#     from FoundryDataBrowser.viewers.trpl_npz import TRPL3dNPZView, TRPLNPZView
...
try:
    from FoundryDataBrowser.viewers.picoharp_mcl_2dslowscan import (
        FiberPicoharpScanView,
        Picoharp_MCL_2DSlowScan_View,
    )
...
try:
    from FoundryDataBrowser.viewers.APD_MCL_2DSlowScanView import (
        APD_MCL_2DSlowScanView,
        APD_MCL_3DSlowScanView,
    )
...
try:
    from FoundryDataBrowser.viewers.WinSpecMCL2DSlowScanView import (
        WinSpecMCL2DSlowScanView,
    )

    app.load_view(WinSpecMCL2DSlowScanView(app))

    from FoundryDataBrowser.viewers.winspec_remote_readout_h5 import (
        WinSpecRemoteReadoutView,
    )
...
try:
    from FoundryDataBrowser.viewers.power_spec_logger_view import (
        PowerSpectrumLoggerView,
    )
...
try:
    from FoundryDataBrowser.viewers.ranged_optimization_h5 import (
        RangedOptimizationH5View,
    )
...
try:
    from FoundryDataBrowser.viewers.odmr_h5 import ODMRH5

    app.load_view(ODMRH5(app))
except Exception as err:
    _on_load_err(err)
