...
class PicoquantHistogramH5View(DataBrowserView):
    ...
    def setup(self):
        ...
        self.plot_n_fit.ui.plot.setLogMode(False, True)

        # data slicers
        plot_data = self.plot_n_fit.ui.data_lines[0]
        ...
        self.plot_n_fit.ui.settings_layout.insertWidget(0, self.x_slicer.New_UI())
        self.plot_n_fit.ui.settings_layout.insertWidget(1, self.bg_slicer.New_UI())
        ...
        self.setdock = self.dockarea.addDock(
            name="Data Settings",
            position="below",
            relativeTo=self.plot_n_fit.ui.settings_dock,
            widget=self.settings.New_UI(),
        )

        # Metadata from file
        self.posible_meta_data = [
            "ElapsedMeasTime",
            "Tacq",
            "Resolution",
            "CountRate0",
        ...
        self.plot_n_fit.ui.settings_dock.setStretch(1, 1)
        self.plot_n_fit.ui.settings_dock.raiseDock()

    @QtCore.Slot()
    def update_display(self):
        x, y = self.get_xy(apply_use_x_slice=False)
        self.plot_n_fit.set_data(x, y, line_number=0, is_data_to_fit=False)

        self.plot_n_fit.update_data_to_fit()
    ...
    def on_change_data_filename(self, fname):
        try:
            ...
            self.histograms = H["time_histogram"][:].reshape(
                -1, len(self.time_array)
            )  # force shape (Nchan, Nbins)
...
def bin_y_average_x(x, y, binning=2, axis=-1, datapoints_lost_warning=True):
    ...
    if not data_loss == 0 and datapoints_lost_warning:
