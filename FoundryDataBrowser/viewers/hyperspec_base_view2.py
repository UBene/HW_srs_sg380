...
class HyperSpectralBaseView(DataBrowserView):
    ...
    def setup(self):
        ...
        self.plot_n_fit = PlotNFit(Ndata_lines=2, colors=self.line_colors)
        ...
        self.corr_dock = self.dockarea.addDock(
            name="correlation", position="right", relativeTo=self.spec_dock
        )
        ...
        self.graph_layout = self.plot_n_fit.ui.graph_layout
        ...
        self.bg_subtract = S.New(
            "bg_subtract", str, initial="None", choices=bg_subtract_choices
        )
        ...
        font.setBold(True)
        self.x_slicer = DataSelector(self.rect_plotdata, "x_slicer", initials=[0, 1, 1])
        self.bg_slicer = DataSelector(
            self.point_plotdata, "bg_slicer", initials=[0, 1, 1]
        )

        self.x_slicer.add_listener(self.update_display)
        self.bg_slicer.add_listener(self.update_display)
        ...
        self.plot_n_fit.ui.add_button("fit_map", self.fit_map)
