...
class ToupCamCamera(object):
    ...
    def pop_image_and_timestamp_fifo(self):
        if len(self.images_with_timestamps)==0:
            return None,None
        return self.images_with_timestamps.pop(0)
    
    def clear_image_and_timestamp_fifo(self):
        self.images_with_timestamps = []
    ...
    def open(self):
        ...
        def get_frame(nEvent, ctx):
            if nEvent == TOUPCAM_EVENT_IMAGE:
                ...
                #print("TOUPCAM_EVENT_IMAGE", time.time())
                self.images_with_timestamps.append((self._data_timestamp, self._data.copy() )  )
                if len(self.images_with_timestamps) > 100:
                    self.images_with_timestamps = self.images_with_timestamps[-100:]
                ...
                #print("TOUPCAM_EVENT_STILLIMAGE", time.time())
