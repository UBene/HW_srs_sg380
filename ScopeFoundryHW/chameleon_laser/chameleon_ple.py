...
class ChameleonPLE(Measurement):
    ...
    def setup(self):
        ...
        self.use_shutter = S.New('use_shutter', dtype=bool, initial=True)

        self.wl_start = S.New('wl_start', dtype=float, initial=700, unit='nm')
        self.wl_stop = S.New('wl_stop',  dtype=float, initial=900, unit='nm')
        self.wl_step = S.New('wl_step', dtype=float, initial=10, unit='nm')

        self.use_ccd = S.New('use_ccd', dtype=bool, initial=True)
        self.ccd_bgsub = S.New('ccd_bgsub', dtype=bool, initial=True)
        self.collect_apd = S.New('collect_apd', dtype=bool, initial=False)

        self.up_down_sweep = S.New('up_down_sweep', dtype=bool, initial=True)

        self.update_period = 0.250
        self.laser_hc = self.app.hardware['ChameleonUltraIILaser']
        self.powermeter = self.app.hardware['thorlabs_powermeter']
        self.shutter = self.app.hardware['shutter_servo']
    ...
    def setup_figure(self):
        ...
        self.plot_current_spec = self.graph_layout.addPlot(
            title="Current Spectrum")
        self.plotline_current_spec = self.plot_current_spec.plot([0])

        self.plot_pm_power = self.graph_layout.addPlot(
            title="PM Power vs laser wl")
    ...
    def update_display(self):
        if self.i_wl:
            i_wl = self.i_wl
            self.plotline_current_spec.setData(
                self.ccd_wls, self.ccd_specs[i_wl])

            self.plotline_pm_power.setData(
                self.wls[0:i_wl], self.pm_powers[0:i_wl])
            self.plotline_pm_power_after.setData(
                self.wls[0:i_wl], self.pm_powers_after[0:i_wl])
    ...
    def _run(self):
        ...
        USE_SHG = True  # When True, the SHG of Chameleon is used for excitation. PM set wl needs to be halfed
        ...
        # in the GUI
        # Note this is the wls of the TiSph main beam
        self.wls = np.arange(
            self.wl_start.val, self.wl_stop.val, self.wl_step.val)
        ...
        if self.up_down_sweep.val:
            # print(self.wls)
            # Note, self.wls is always the wls of the TiSph main beam
            self.wls = np.concatenate([self.wls, self.wls[::-1]])
            print(self.wls)
        ...
        try:
            ...
            for i_wl, wl in enumerate(self.wls):
                ...
                # SHG needs about 20s to stablize!!!#####Main bean needs only ~5s
                time.sleep(20)
                ...
                if SET_POWER_WHEEL_POS:
                    new_pos = int(
                        np.interp(wl, power_wheel_positions[0], power_wheel_positions[1]))
            ...
            if SET_POWER_WHEEL_POS:
                save_dict.update(
                    {'power_wheel_positions': self.power_wheel_positions})
