...
class uc480:
    ...
    def connect(self, ID=0, useDevID=False):
        ...
        self.get_exposure_limits()
        self.get_pixelclock_range()
    ...
    def get_exposure_range(self):
        return self.get_exposure_limits()
    ...
    def get_pixelclock(self):
        ...
        nRet = is_PixelClock(hCam, IS_PIXELCLOCK_CMD_GET, (void*)&nPixelClock, sizeof(nPixelClock));
        """
        nPixelClock = ctypes.c_int()
        self.call('is_PixelClock', self._camID, IS_PIXELCLOCK_CMD_GET, ptr(nPixelClock), ctypes.sizeof(nPixelClock))
        return nPixelClock.value
    
    def set_pixelclock(self, pxclock):
        """Sets the pixel clock in MHz
        Note this will change range of allowed frame rates and exposures
        """
        pParam = ctypes.c_int(pxclock)
        self.call('is_PixelClock', self._camID, IS_PIXELCLOCK_CMD_SET, ptr(pParam), ctypes.sizeof(pParam))
    
    def get_pixelclock_range(self):
        """Returns the supported limits for the pixelclock (*min, max, increment*).
        """        
        intRange = (ctypes.c_uint * 3)()
        self.call("is_PixelClock", self._camID, IS_PIXELCLOCK_CMD_GET_RANGE, ptr(intRange), ctypes.sizeof(intRange))
        print("Valid pixel clocks: %iMHz to %iMHz in steps of %iMHz" % (intRange[0], intRange[1], intRange[2]))
        #self.expmin, self.expmax, self.expinc = dblRange
        return tuple(intRange)       
