...
class Artemis_Enum(object):
    
    def __init__(self, name, prefix, name_num_pairs):
        self.name = name
        self.prefix = prefix
        self.byfullname = OrderedDict(name_num_pairs)
        self.byname = OrderedDict() # ShortName
        self.bynum_fullname  = OrderedDict()
        self.bynum  = OrderedDict()
        for name, num in self.byfullname.items():
            self.bynum_fullname[num] = name
            short_name = name[len(prefix):]
            assert name[len(prefix):] == prefix
            self.byname[short_name] = num
            self.bynum[num] = short_name
...
"""
ARTEMISERROR_Enum = Artemis_Enum(
    "ARTEMISERROR", "ARTEMIS_",
    [
    ("ARTEMIS_OK",0),
...
"""
ARTEMISCOLOURTYPE_Enum = Artemis_Enum(
    "ARTEMISCOLOURTYPE", "ARTEMIS_COLOUR_",
    [("ARTEMIS_COLOUR_UNKNOWN", 0)
     ("ARTEMIS_COLOUR_NONE", 1)
    ("ARTEMIS_COLOUR_RGGB", 2)])
...
ARTEMISPRECHARGEMODE_Enum = Artemis_Enum(
    "ARTEMISPRECHARGEMODE", "PRECHARGE_",
    [("PRECHARGE_NONE", 0),        #// Precharge ignored
    ("PRECHARGE_ICPS", 1),         #// In-camera precharge subtraction
    ("PRECHARGE_FULL", 2),         #// Precharge sent with image data
    ])
...
"""
ARTEMISCAMERASTATE_Enum = Artemis_Enum(
    "ARTEMISCAMERASTATE", "CAMERA_",
    [("CAMERA_ERROR",-1),
     ("CAMERA_IDLE",0),
     ("CAMERA_WAITING",1),
     ("CAMERA_EXPOSING",2),
...
"""
ARTEMISSENDMSG_Enum = Artemis_Enum(
    "ARTEMISSENDMSG", "ARTEMIS_",
    [
    ("ARTEMIS_LE_LOW"                , 0),
    ("ARTEMIS_LE_HIGH"                , 1),
    ("ARTEMIS_GUIDE_NORTH"            , 10),
...
"""
ARTEMISPROCESSING_Enum = Artemis_Enum(
    "ARTEMISPROCESSING", "ARTEMIS_PROCESS_",
    [
    ("ARTEMIS_PROCESS_LINEARISE"    ,1),    #// compensate for JFET nonlinearity
    ("ARTEMIS_PROCESS_VBE"          ,2),    #// adjust for 'Venetian Blind effect'
    ])
...
"""
ARTEMISSETUPADC_Enum = Artemis_Enum(
    "ARTEMISSETUPADC", "ARTEMIS_SETUPADC_",
    [
    ("ARTEMIS_SETUPADC_MODE"       , 0     ),
    ("ARTEMIS_SETUPADC_OFFSETR"    ,(1<<10)),
    ("ARTEMIS_SETUPADC_OFFSETG"    ,(2<<10)),
...
"""
ARTEMISPROPERTIESCCDFLAGS_Enum = Artemis_Enum(
    "ARTEMISPROPERTIESCCDFLAGS", "ARTEMIS_PROPERTIES_CCDFLAGS_" ,
    [
    ("ARTEMIS_PROPERTIES_CCDFLAGS_INTERLACED" ,1), #// CCD is interlaced type
    ("ARTEMIS_PROPERTIES_CCDFLAGS_DUMMY",0x7FFFFFFF) #// force size to 4 bytes
    ])
...
"""
ARTEMISPROPERTIESCAMERAFLAGS_Enum = Artemis_Enum(
    "ARTEMISPROPERTIESCAMERAFLAGS", "ARETMIS_PROPERTIES_CAMERAFLAGS_",
    [
    ("ARETMIS_PROPERTIES_CAMERAFLAGS_FIFO" , 1), #// Camera has readout FIFO fitted
    ("ARETMIS_PROPERTIES_CAMERAFLAGS_EXT_TRIGGER" , 2), #// Camera has external trigger capabilities
    ("ARETMIS_PROPERTIES_CAMERAFLAGS_PREVIEW" , 4), #// Camera can return preview data
    ("ARETMIS_PROPERTIES_CAMERAFLAGS_SUBSAMPLE" , 8), #// Camera can return subsampled data
    ("ARETMIS_PROPERTIES_CAMERAFLAGS_HAS_SHUTTER" , 16), #// Camera has a mechanical shutter
    ("ARETMIS_PROPERTIES_CAMERAFLAGS_HAS_GUIDE_PORT" , 32), #// Camera has a guide port
    ("ARETMIS_PROPERTIES_CAMERAFLAGS_HAS_GPIO" , 64), #// Camera has GPIO capability
    ("ARETMIS_PROPERTIES_CAMERAFLAGS_HAS_WINDOW_HEATER" , 128), #// Camera has a window heater
    ("ARETMIS_PROPERTIES_CAMERAFLAGS_HAS_EIGHT_BIT_MODE" , 256), #// Camera can download 8-bit images
    ("ARETMIS_PROPERTIES_CAMERAFLAGS_HAS_OVERLAP_MODE" , 512), #// Camera can overlap
    ("ARETMIS_PROPERTIES_CAMERAFLAGS_HAS_FILTERWHEEL" , 1024), #// Camera has internal filterwheel
    ("ARETMIS_PROPERTIES_CAMERAFLAGS_DUMMY" , 0x7FFFFFFF) #// force size to 4 bytes
    ])
...
artfn  bool ArtemisOpenDevice(int Device);
artfn  bool ArtemisCloseDevice(int Device);
artfn  int ArtemisRefreshDevices(int Device);

artfn  bool ArtemisDevicePresent(int Device);
