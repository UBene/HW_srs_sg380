...
class TimeHarpOptimizerMeasure(Measurement):
    ...
    def setup(self):
        ...
        for i in range(self.n_channels):
            self.rates.append('CountRate{}'.format(i))
        for rate in self.rates:
            self.settings.New('{}_visible'.format(rate),
    ...
    def setup_figure(self):
        ...
        self.settings.SyncRate_visible.connect_to_widget(
            self.ui.SyncRate_visible_checkBox)
        ...
        for i, rate in enumerate(self.rates):
            ...
            avg_line = pg.InfiniteLine(
                angle=0, movable=False, pen=colors[i % len(colors)])
    ...
    def add_timing_module(self, channels=[0, 1]):
        ...
        for i in channels:
            ...
            module_layout.addWidget(
                self.hydraharp_hw.settings.New_UI(include=include))
    ...
    def run(self):
        ...
        while not self.interrupt_measurement_called:
            ...
            self.optimize_history[self.optimize_ii, :] = [
                hh_hw.settings[rate] for rate in self.rates]
            time.sleep(0.08)
    ...
    def update_display(self):
        ...
        if q >= 0:
            ...
            avg_val = self.optimize_history[:ii].mean(
                axis=0) * 1.0 * ii / N_AVG + self.optimize_history[q:].mean(axis=0) * (-q / N_AVG)

        for i, rate in enumerate(self.rates):
            self.plotlines[rate].setVisible(
                self.settings['{}_visible'.format(rate)])
            self.avglines[rate].setVisible(
                self.settings['{}_visible'.format(rate)])
            if self.settings['{}_visible'.format(rate)]:
                title += '<b>\u03BC</b><sub>{}</sub> = {:3.0f} <br />'.format(
                    rate, avg_val[i])
