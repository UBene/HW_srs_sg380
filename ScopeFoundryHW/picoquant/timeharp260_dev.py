...
class TimeHarp260:
    ...
    def __init__(self, devnum=0, mode='HIST', refsource='internal', debug=False):
        ...
        self.hhlib = ctypes.WinDLL("th260lib64.dll")
        ...
        try:
            ...
            with self.lock:
                self._err(self.hhlib.TH260_Initialize(self.devnum, 
...
#             with self.lock:
#                 self._err(self.hhlib.TH260_GetHardwareInfo(self.devnum,hw_model,hw_partno)) #/*this is only for information*/
...
#             if self.debug: print("Found Model %s Part No %s" % (self.hw_model, self.hw_partno))
            ...
            s = c_int()
            with self.lock:
                self._err(self.hhlib.TH260_GetBaseResolution(self.devnum, byref(r), byref(s)))
            self.base_resolution = r.value # in picoseconds
            self.max_bin_steps = s.value
            
            # TH260_GetNumOfInputChannels
            x = c_int()
            self._err(self.hhlib.TH260_GetNumOfInputChannels(self.devnum, byref(x)))
            self.num_input_channels = x.value
            
            # TH260_GetNumOfModules
            #x = c_int()
            #self._err(self.hhlib.TH260_GetNumOfModules(self.devnum, byref(x)))
    ...
    def close(self):
        with self.lock:
            self._err(self.hhlib.TH260_CloseDevice(self.devnum))

    def _err(self, retcode):
        if retcode < 0:
            err_buffer = create_string_buffer(40)
            self.hhlib.TH260_GetErrorString(err_buffer, retcode)
            self.err_message = err_buffer.value.decode()
            raise IOError("TimeHarp260 Error {}: {}".format(retcode, self.err_message))
        return retcode
    ...
    def SetSyncDiv(self, div):
        ...
        with self.lock:
            self._err(self.hhlib.TH260_SetSyncDiv(self.devnum, int(div)))
    
    
    def SetSyncCFD(self, level, zerocross):
        """level and zerocross in integer millivolts"""
        with self.lock:
            self._err(self.hhlib.TH260_SetSyncCFD(self.devnum, int(level), int(zerocross)))
            
    def SetSyncChannelOffset(self, value):
        "value: sync timing offset in ps"
        with self.lock:
            self._err(self.hhlib.TH260_SetSyncChannelOffset(self.devnum, int(value)))
    ...
    def SetInputCFD(self, chan, level, zerocross):
        ...
        with self.lock:
            self._err(self.hhlib.TH260_SetInputCFD(self.devnum, chan, int(level), int(zerocross)))
    
    def SetInputChannelOffset(self, chan, value):
        "value: sync timing offset in ps"
        assert 0 <= chan < self.num_input_channels
        with self.lock:
            self._err(self.hhlib.TH260_SetInputChannelOffset(self.devnum, chan, int(value)))
    
    def SetInputChannelEnable(self, chan, enable):
        assert 0 <= chan < self.num_input_channels
        with self.lock:
            self._err(self.hhlib.TH260_SetInputChannelEnable(
                self.devnum, chan, int(bool(enable))))

    def SetStopOverflow(self, stop_ofl=True, stopcount=STOPCNTMAX):
        with self.lock:
            self._err(self.hhlib.TH260_SetStopOverflow( int(self.devnum), 
    ...
    def SetBinning(self, binning):
        ...
        """
        with self.lock:
            self._err(self.hhlib.TH260_SetBinning(self.devnum, int(binning)))
    ...
    def SetHistoLen(self, lencode=MAXLENCODE):
        ...
        x = c_int()
        with self.lock:
            self._err(self.hhlib.TH260_SetHistoLen(self.devnum, lencode, byref(x)))
    ...
    def ClearHistMem(self):
        with self.lock:
            self._err(self.hhlib.TH260_ClearHistMem(self.devnum))
    ...
    def StartMeas(self, tacq):
        """tacq: acquisition time in integer milliseconds
                ...
                maximum = ACQTMAX"""
        with self.lock:
            self._err(self.hhlib.TH260_StartMeas(self.devnum, int(tacq)))
        
    def StopMeas(self):
        """Note: Can also be used before the acquisition time expires."""
        with self.lock:
            self._err(self.hhlib.TH260_StopMeas(self.devnum))
    ...
    def CTCStatus(self):
        ...
        status = c_int()
        with self.lock:
            self._err(self.hhlib.TH260_CTCStatus(self.devnum, byref(status)))
    ...
    def read_histogram_data(self, channel=0, clear_after=False):
        ...
        with self.lock:
            self._err(self.hhlib.TH260_GetHistogram(self.devnum, 
    ...
    def GetResolution(self):
        ...
        r = c_double(0)
        with self.lock:
            self._err(self.hhlib.TH260_GetResolution(self.devnum, byref(r)))
    ...
    def GetSyncRate(self):
        ...
        r = c_int(0)
        with self.lock:
            self._err(self.hhlib.TH260_GetSyncRate(self.devnum, byref(r)))
        return r.value     
    ...
    def GetCountRate(self, chan):
        ...
        r = c_int(0)
        with self.lock:
            self._err(self.hhlib.TH260_GetCountRate(self.devnum, chan, byref(r)))
        return r.value
    ...
    def GetElapsedMeasTime(self):
        ...
        x = c_double(0)
        with self.lock:
            self._err(self.hhlib.TH260_GetElapsedMeasTime(self.devnum, byref(x)))
        return x.value
    ...
    def GetSyncPeriod(self):
        ...
        x = c_double(0)
        with self.lock:
            self._err(self.hhlib.TH260_GetSyncPeriod(self.devnum, byref(x)))
        return x.value        
