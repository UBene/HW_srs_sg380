...
class ThorlabsPM100D:
    ...
    def __init__(self, port="USB0::0x1313::0x8078::P0005750::INSTR", debug=False, logger=logging.getLogger(__name__)):
        ...
        if debug:
            self.logger.debug(self.visa_resource_manager.list_resources())
    ...
    def ask(self, cmd):
        if self.debug:
            self.logger.debug("PM100D ask " + repr(cmd))
        with self.lock:
            try:
        ...
        if self.debug:
            self.logger.debug("PM100D resp ---> " + repr(resp))
        return resp

    def write(self, cmd):
        if self.debug:
            self.logger.debug("PM100D write" + repr(cmd))
        with self.lock:
            resp = self.pm.write(cmd)
        if self.debug:
            self.logger.debug("PM100D written --->" + repr(resp))
    ...
    def get_wavelength(self):
        ...
        while True:
            try:
                ...
                if self.debug:
                    self.logger.debug("wl:" + repr(self.wl))
                break
            except:
                if try_count >= TRIES_BEFORE_FAILURE:
                    self.logger.warning("Failed to get wavelength.")
                    break
                else:
                    time.sleep(RETRY_SLEEP_TIME)  # take a rest..
                    try_count = try_count + 1
                    self.logger.debug("trying to get the wavelength again..")
        return self.wl
    ...
    def set_wavelength(self, wl):
        ...
        while True:
            try:
                ...
                if try_count >= TRIES_BEFORE_FAILURE:
                    self.logger.warning("Failed to set wavelength.")
                    ...
                    time.sleep(RETRY_SLEEP_TIME)  # take a rest..
                    try_count = try_count + 1
                    self.logger.warning("trying to set wavelength again..")
    ...
    def get_attenuation_dB(self):
        ...
        if self.debug:
            self.logger.debug("attenuation_dB " + repr(self.attenuation_dB))
    ...
    def get_average_count(self):
        ...
        if self.debug:
            self.logger.debug("average count:" + repr(self.average_count))
    ...
    def measure_power(self):
        ...
        if self.debug:
            self.logger.debug("power: " + repr(self.power))
        return self.power
    ...
    def get_power_range(self):
        ...
        if self.debug:
            self.logger.debug("power_range " + repr(self.power_range))
    ...
    def get_auto_range(self):
        ...
        if True:
            self.logger.debug('get_auto_range ' + repr(resp))
        self.auto_range = bool(int(resp))
        return self.auto_range

    def set_auto_range(self, auto=True):
        self.logger.debug("set_auto_range " + repr(auto))
        if auto:
    ...
    def measure_frequency(self):
        ...
        if self.debug:
            self.logger.debug("frequency " + repr(self.frequency))
        return self.frequency

    def get_zero_magnitude(self):
        resp = self.ask("SENS:CORR:COLL:ZERO:MAGN?")
        if self.debug:
            self.logger.debug("zero_magnitude " + repr(resp))
    ...
    def get_zero_state(self):
        ...
        if self.debug:
            self.logger.debug("zero_state" + repr(resp))
    ...
    def get_photodiode_response(self):
        ...
        if self.debug:
            self.logger.debug("photodiode_response (A/W)" + repr(resp))
    ...
    def measure_current(self):
        ...
        if self.debug:
            self.logger.debug("measure_current " + repr(resp))
    ...
    def get_current_range(self):
        ...
        if self.debug:
            self.logger.debug("current_range (A)" + repr(resp))
