...
class AndorCCDHW(HardwareComponent):
    def setup(self):
        ...
        self.status = self.settings.New(
            name="ccd_status", dtype=str, initial="?", fmt="%s", ro=True
        )

        self.temperature = self.settings.New(
            name="temperature",
            dtype=int,
            ro=True,
            unit="C",
        ...
        self.settings.New(
            "temp_setpoint", dtype=int, unit="C", vmin=-300, vmax=300, si=False
        )
        self.settings.New(
            "temp_status", dtype=str, ro=True, initial="?",
        )

        self.cooler_on = self.settings.New(
            name="cooler_on", dtype=bool, ro=False, initial=True
        )

        self.exposure_time = self.settings.New(
            name="exposure_time",
            dtype=float,
            spinbox_decimals=4,
            fmt="%e",
        ...
        self.em_gain = self.settings.New(
            "em_gain", dtype=int, ro=False, si=False, vmin=1, vmax=4096
        )

        self.acq_mode = self.settings.New(
            "acq_mode",
            dtype=str,
            initial="single",
        ...
        self.acc_time = self.settings.New(
            "acc_time", dtype=float, unit="s", initial=0.1, si=True
        )
        self.kin_time = self.settings.New(
            "kin_time", dtype=float, unit="s", initial=0.1, si=True
        )
        self.num_acc = self.settings.New("num_acc", dtype=int, initial=1, vmin=1)
        self.num_kin = self.settings.New("num_kin", dtype=int, initial=1, vmin=1)

        # Output amplifier ( EMCCD or conventional)
        self.output_amp = self.settings.New(
            "output_amp",
            dtype=int,
            ro=False,
        ...
        # Readout mode
        self.readout_mode = self.settings.New(
            name="readout_mode",
            dtype=str,
            ro=False,
            initial="Image",
        ...
        self.roi_img_hstart = self.settings.New(
            "roi_img_hstart", dtype=int, unit="px", ro=False, initial=1
        )
        self.roi_img_hend = self.settings.New(
            "roi_img_hend", dtype=int, unit="px", ro=False, initial=512
        )
        self.roi_img_hbin = self.settings.New(
            "roi_img_hbin", dtype=int, unit="px", ro=False, initial=1
        )
        self.roi_img_vstart = self.settings.New(
            "roi_img_vstart", dtype=int, unit="px", initial=1, ro=False
        )
        self.roi_img_vend = self.settings.New(
            "roi_img_vend", dtype=int, unit="px", initial=512, ro=False
        )
        self.roi_img_vbin = self.settings.New(
            "roi_img_vbin", dtype=int, unit="px", initial=1, ro=False
        )

        # ROI Parameters for single track readout mode
        self.roi_st_center = self.settings.New(
            "roi_st_center", dtype=int, unit="px", ro=False, initial=256
        )
        self.roi_st_width = self.settings.New(
            "roi_st_width", dtype=int, unit="px", ro=False, initial=10
        )
        self.roi_st_hbin = self.settings.New(
            "roi_st_hbin", dtype=int, unit="px", ro=False, initial=1
        )

        # ROI parameters for full vertical binning
        self.roi_fvb_hbin = self.settings.New(
        ...
        self.hflip = self.settings.New("hflip", dtype=bool, initial=True)
        self.vflip = self.settings.New("vflip", dtype=bool, initial=False)
    ...
    def connect(self):
        ...
        self.settings.temp_setpoint.connect_to_hardware(
            write_func=self.ccd_dev.set_temperature
        )
        self.settings.temp_setpoint.write_to_hardware()
        self.settings.temp_status.connect_to_hardware(
            self.ccd_dev.get_temperature_status
        )
        ...
        self.hs_speed_conventional.hardware_set_func = (
            self.ccd_dev.set_hs_speed_conventional
        )
        ...
        if self.settings["has_em_ccd"]:
            ...
            for chan_i in range(self.ccd_dev.numADChan):
                for speed_i, speed in enumerate(self.ccd_dev.HSSpeeds_EM[chan_i]):
                    shift_speed_names[speed_i] = shift_speed_names.get(
                        speed_i, ""
        ...
        for speed_i in range(self.ccd_dev.numVSSpeeds):
            choices.append(
                (
                    str.format(
                        "Speed {} - {:.2f} us", speed_i, self.ccd_dev.VSSpeeds[speed_i]
                    ),
                    speed_i,
                )
            )
    ...
    def is_background_valid(self):
        ...
        if bg is not None:
            if bg.shape == self.ccd_dev.buffer.shape:
                ...
                self.log.debug(
                    "Background not the correct shape {} {}".format(
                        self.ccd_dev.buffer.shape, bg.shape
                    )
                )
        else:
    ...
    def set_readout(self):
        ...
        if ro_mode == "FullVerticalBinning":
            ...
            self.ccd_dev.set_ro_single_track(
                self.roi_st_center.val, self.roi_st_width.val, self.roi_st_hbin.val
            )
        else:
    ...
    def read_temp_op(self):
        ...
        self.log.debug(
            "get_temperature_range: {}".format(self.ccd_dev.get_temperature_range())
        )
