...
from ScopeFoundryHW.lightfield.lightfield_dev_IR import LightFieldDev_IR


class LightFieldHW_IR(HardwareComponent):

    def setup(self):
        self.name = "lightfield_IR"
        self.debug = True
        self.background = None

        self.correct_background = self.add_logged_quantity(name="correct_background", dtype=bool, ro=False,
                                                           initial=False)
        ...
        #                                             choices=[("Low Noise", "Low Noise"),
        #                                                      ("Electron Multiplied", "Electron Multiplied")]
        #                                             )

        # self.adc_speed = self.add_logged_quantity("adc_speed", dtype=float, ro=False,
        #                                           choices=[("500 kHz", 0.5), ("2 MHz", 2.0)]
        #                                           , initial=0.5)
        ...
        # self.custom_roi_y = self.add_logged_quantity("custom_roi_y", dtype=int, ro=False, initial=0)
        # self.custom_roi_width = self.add_logged_quantity("custom_roi_width", dtype=int, ro=False, initial=1024)
        # self.custom_roi_height = self.add_logged_quantity("custom_roi_height", dtype=int, ro=False, initial=1)
        ...
        #                                                  dtype=int,
        #                                                  choices=[('2 us', 2), ('3 us', 3), ('4 us', 4), ('6 us', 6)],
        #                                                  initial=6)
        ...
        self.shutter_mode = self.add_logged_quantity("shutter_mode", dtype=str, ro=False,
                                                     ...
                                                     , initial="Normal")
        self.shutter_op_delay = self.add_logged_quantity("shutter_op_delay", dtype=int, unit="ms", si=False, ro=False,
                                                         initial=20)
        self.shutter_cl_delay = self.add_logged_quantity("shutter_cl_delay", dtype=int, unit="ms", si=False, ro=False,
                                                         initial=20)
    ...
    def connect(self):
        if self.debug: print("Connecting to LightField")

        # Open connection to hardware
        self.lightfield_dev = LightFieldDev_IR(debug=self.debug)
        ...
        # self.roi_type.hardware_set_func = self.lightfield_dev.set_roitype # Do not exist
        # self.x_binning.hardware_read_func = self.lightfield_dev.get_XBinning # Do not exist
        # self.x_binning.hardware_set_func = self.lightfield_dev.set_XBinning # Do not exist
        # self.y_binning.hardware_read_func = self.lightfield_dev.get_YBinning # Do not exist
        # self.y_binning.hardware_set_func = self.lightfield_dev.set_YBinning # Do not exist
        ...
        #     self.custom_roi_y.update_value(0)
        #     self.custom_roi_width.update_value(1024)
        #     self.custom_roi_height.update_value(1)
            # self.custom_roi_xbinning.update_value(1)
            # self.custom_roi_ybinning.update_value(1)

        print('Arrived stage 3')
            #######################################################
