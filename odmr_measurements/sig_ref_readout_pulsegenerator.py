...
class SigRefReadoutProgramGenerator(PulseProgramGenerator):
    ...
    def make_pulse_channels(self):
        ...
        time_5_MHz_start = np.copy(
            times_10MHz[len(times_10MHz) - 1]) + base_tick_period / 4

        periods_5MHz = (total_periods / 3) * 2
        times_5MHz = np.arange(periods_5MHz) * \
            (base_tick_period / 2) + time_5_MHz_start
        time_2_5_MHz_start = np.copy(
            times_5MHz[len(times_5MHz) - 1]) + base_tick_period / 2

        times_2_5MHz = np.copy(np.arange(total_periods / 3)
                               * (base_tick_period) + time_2_5_MHz_start)
        ...
        DAQ_sig = self.new_channel('DAQ_sig', [t_readout_sig], [t_gate])
