...
class PolarizationScanMeasure(Measurement):
    ...
    def setup(self):          #SF build in fct, create logged quantity
        ...
        self.Polarization_Compensation_Ratio = self.add_logged_quantity("Polar_Compen_ratio",
                                                        dtype=float, spinbox_decimals=3, initial=0, vmin=-100, vmax=+100, ro=False)
    ...
    def setup_figure(self):        #SF build in fct, connect lq to the UI
        ...
        self.IntegrationRange_min.connect_to_widget(self.ui.IntegrationRange_min_spinBox)
        self.IntegrationRange_max.connect_to_widget(self.ui.IntegrationRange_max_spinBox)
        ...
        self.plot1 = self.graph_layout.addPlot(title="Polarization Scan")
    ...
    def update_display(self):
        ...
        #if self.settings['x_axis'] == 'Excitation_Stage_Pos':           #
        if self.x_axis_flag == 0:
            if hasattr(self, 'Excitation_Stage_position'):
                X = self.Excitation_Stage_position[:ii]
        elif self.x_axis_flag == 1:
            #elif self.settings['x_axis'] == 'Collection_Stage_Pos':
            if hasattr(self, 'Collection_Stage_position'):
                X = self.Collection_Stage_position[:ii]
        ...
        if self.settings['collect_apd']:
            ...
            if hasattr(self, 'Si_pm_reading'):
                self.plot_line1.setData(X, self.Si_pm_reading[:ii])
        elif self.settings['collect_Ge_powermeter']:
            if hasattr(self, 'Ge_pm_reading'):
                self.plot_line1.setData(X, self.Ge_pm_reading[:ii])
    ...
    def run(self):
        ...
        self.ratio = self.Polarization_Compensation_Ratio/100*self.step_size_Excitaion/10
        ...
        if self.Fix_Excitation_Stage.value == True:
            ...
            self.x_axis_flag = 1
            self.ratio = self.Polarization_Compensation_Ratio.val/100*self.step_size_Collection/10
        ...
        for ii in range(self.Np):
            ...
            if self.settings['collect_spectrum']:
                ...
                if self.Int_max == 0 & self.Int_min == 0 :
                    self.Int_min = min(range(len(wls)), key=lambda i: abs(wls[i]-self.IntegrationRange_min))   #Finde the index of desired min/max in wls
                    self.Int_max = min(range(len(wls)), key=lambda i: abs(wls[i]-self.IntegrationRange_max))
