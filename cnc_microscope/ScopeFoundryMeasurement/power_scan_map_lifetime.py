...
class PowerScanMap2D_lifetime(Measurement):
    name = "power_scan_map_lifetime"
    ...
    def setup(self):
        self.ui_filename = sibling_path(__file__,"power_scan_map_lifetime.ui")
    ...
    def pre_scan_setup(self):
        ...
        if self.settings['collect_apd']:
            ...
            print ('*************Set to collect CCD image')
            self.hh_time = np.zeros((1, self.Nv.val, self.Nh.val, 65536), dtype=float)
            self.hh_histograms = np.zeros((1, self.Nv.val, self.Nh.val, 65536), dtype=float)
            self.popt = np.zeros((1, self.Nv.val, self.Nh.val, 4), dtype=float)
            self.pcov = np.zeros((1, self.Nv.val, self.Nh.val, 4, 4), dtype=float)
            self.lifetime_graph = np.zeros((1, self.Nv.val, self.Nh.val), dtype=float)
            self.elapsed_time = np.zeros((1, self.Nv.val, self.Nh.val), dtype=float)
            self.apd_ct = np.zeros((1, self.Nv.val, self.Nh.val), dtype=float)
    ...
    def update_display(self):
        ...
        if self.initial_scan_setup_plotting:
            ...
            if self.settings['collect_apd']:
                ...
                self.disp_img = self.lifetime_graph[kk,:,:].T
...
#             print "for loop raster gen", time.time() - t0
    ...
    def run(self):
        ...
        try:
            ...
            #print("scan_index_array:{}".format(self.scan_index_array))
            ...
            for self.pixel_i in range(self.Npixels):
                ...
                kk, jj, ii = self.current_scan_index
                #print("kk,jj,ii: {},{},{}".format(kk,jj,ii))
    ...
    def collect_pixel(self, pixel_num, k, j, i):
        ...
        if self.settings['collect_apd']:      
            ...
            self.count_rate_map[k, j, i] = count_reading  # changed from [k, i, j] to [k, j, i] by Kaiyuan, 07/24/2018, to fix data recording and  problems.
        ...
        if self.settings["collect_lifetime"]:
            ...
            try:
                popt0, pcov0 = curve_fit(self.biexpfunc1, time_array[0:number_point]/10**9, hist_data0[0:number_point], (0.5,10.0,1.0,0.0),\
                                         bounds=((-0.5,0.0,-1.0,-1.0),(2.0,100000.0,100.0,1.0)), max_nfev = 100000)
            except:
            ...
            if popt_bool == True:
                if np.size(popt0) > 4:
                    ...
                    print("index: ",index)
                    self.lifetime1 = popt0[index]
            print("histogram: {}".format(hist_data0))
            self.hh_time[k,j,i,:] = time_array
            self.hh_histograms[k,j,i,:] = hist_data0
            self.popt[k,j,i,:] = popt0
            self.pcov[k,j,i,:,:] = pcov0
            self.lifetime_graph[k,j,i] = self.lifetime1
            self.elapsed_time[k,j,i] = int(set_time)
            self.apd_ct[k,j,i] = ct
    ...
    def post_scan_cleanup(self):
        ...
        pass
    def biexpfunc1(self, x, a1, b1, c1, d1):
        return b1*np.exp(-(x+d1)/a1) + c1