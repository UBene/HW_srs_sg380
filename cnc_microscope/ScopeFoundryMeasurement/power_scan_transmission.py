...
class PowerScanTran(Measurement):
    
    name = 'power_scan_tran'
    
    def __init__(self, app):
        self.ui_filename = sibling_path(__file__, "power_scan_transmission_2.ui")
    ...
    def setup(self):
        ...
        self.powermeter_ex_type = self.settings.New('powermeter_ex_type', dtype=str, initial='Si',
                                                  choices=('Si', 'Ge'))
        self.powermeter_tr_type = self.settings.New('powermeter_tr_type', dtype=str, initial='Si',
                                                  choices=('Si', 'Ge'))
    ...
    def setup_figure(self):
        ...
        self.powermeter_tr_type.connect_to_widget(self.ui.powermeter_tr_comboBox)
        self.powermeter_ex_type.connect_to_widget(self.ui.powermeter_ex_comboBox)
        ...
        self.plot2 = self.graph_layout.addPlot(title="Transmission")
    ...
    def update_display(self):
        ...
        self.plot_line2.setData(X, self.pm_powers_tr[:ii])
...
#        self.pm_powers_after = np.zeros(Np, dtype=float)
        ...
        for ii in range(self.Np):
            ...
            self.pm_powers_tr.append(self.collect_pm_power_tr_data())
            ...
            self.pm_powers_after_tr.append(self.collect_pm_power_tr_data())
        ...
        try:
            ...
            H['pm_powers_tr'] = np.array(self.pm_powers_tr)
            H['pm_powers_tr_after'] = np.array(self.pm_powers_tr_after)
        ...
        for samp in range(0, PM_SAMPLE_NUMBER):
            ...
            while not self.interrupt_measurement_called:
                try:
                    ...
                    powermeter_hw_name = 'thorlabs_powermeter_' + self.powermeter_ex_type.val
                    while samp_count < 6:
    ...
    def collect_pm_power_tr_data(self):
        PM_SAMPLE_NUMBER = 10
        ...
        for samp in range(0, PM_SAMPLE_NUMBER):
            ...
            while not self.interrupt_measurement_called:
                try:
                    #############Note: Need to Manually Change Which Powermeter to Use Here: Si or Ge
                    powermeter_hw_name = 'thorlabs_powermeter_' + self.powermeter_tr_type.val
                    while samp_count < 6:
                        pm_power = pm_power + self.app.hardware[powermeter_hw_name].power.read_from_hardware(send_signal=True)
                        samp_count = samp_count + 1
                    break 
