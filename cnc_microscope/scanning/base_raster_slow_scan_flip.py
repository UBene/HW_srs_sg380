...
class BaseRaster2DSlowScan(BaseRaster2DScan):
    ...
    def run(self):
        ...
        try:
            ...
            if self.settings['save_h5']:
                if self.settings['collect_apd']:
                    H['count_rate_map' ] = np.flip(self.count_rate_map, 1)
                elif self.settings['collect_spectrum']:
                    H["wls"] = self.wls
                    H["spectra_map"] = np.flip(self.spectra_map, 1)
                    H["integrated_spectra_map"] = np.flip(self.integrated_spectra_map, 1)
                elif self.settings['collect_CCD_image']:
                    H["wls"] = self.wls
                    H["spectra_map"] = np.flip(self.spectra_map, 1)
                    H["integrated_spectra_map"] = np.flip(self.integrated_spectra_map, 1)
                    H["image_map"] = np.flip(self.image_map, 1) 
