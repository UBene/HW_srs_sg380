...
class BaseRaster2DSlowScan(BaseRaster2DScan):
    ...
    def run(self):
        ...
        if self.scan_type.val == "columbia":
        ...
        if self.settings['save_h5']:
            if self.settings['collect_apd']:
                ...
                self.name = 'PI_2DScan_AmCam_image'
        ...
        try:
            ...
            for self.pixel_i in range(self.Npixels):
                ...
                # Open the shutter when scan type is "columbia" by Changhwan on 10/20/19 
                ...
                if self.scan_type.val == "columbia_negative":    
...
#                     self.app.hardware.dual_position_slider_2.move_fwd()
                ...
                #
                # Close the shutter when scan type is "columbia" by Changhwan on 10/20/19 
            ...
            if self.settings['save_h5']:
                if self.settings['collect_apd']:
                    ...
                    H["time_array"] = self.time_array_hh
                    H["hist_data0"] = self.hist_data0
                elif self.settings['collect_toupcam']:
                    H["R"] = self.R_mat
                    H["G"] = self.G_mat
                    H["B"] = self.B_mat
#                     len_x = np.shape(self.time_array_hh)[2]
#                     len_y = np.shape(self.time_array_hh)[1]
#                     
#                     lifetime_array = np.zeros((len_y, len_x, 3))
#                     lifetime_std_array = np.zeros((len_y, len_x, 3))
#                     lifetime_biexp_array = np.zeros((len_y, len_x, 5))
#                     lifetime_biexp_std_array = np.zeros((len_y, len_x, 5))
#                     start_time = 259.0
#                     end_time = 12*10**3 
                    
#                     for i in np.arange(len_y):
#                         for j in np.arange(len_x):
#                             time_i = np.array(self.time_array_hh[0, i, j, :])
#                             hist_data_i = np.array(self.hist_data0[0, i, j, :])
#                             index_start = np.argmin(np.abs(time_i - start_time))
#                             index_end = np.argmin(np.abs(time_i - end_time))
#                             popt0, pcov0 = curve_fit(self.exponential_function, time_i[index_start:index_end]-start_time, hist_data_i[index_start:index_end], 
#                                                      bounds = ([0, 100, 0], [np.inf, 12*10**3, np.inf]))
#                             popt0_bi, pcov0_bi = curve_fit(self.biexponential_function, time_i[index_start:index_end]-start_time, hist_data_i[index_start:index_end], 
#                                                      bounds = ([0, 100, 0, 100, 0], [np.inf, 12*10**3, np.inf, 12*10**3, np.inf]))
#                             perr = np.sqrt(np.diag(pcov0))
#                             perr_bi = np.sqrt(np.diag(pcov0_bi))
#                             lifetime_array[i,j] = popt0
#                             lifetime_std_array[i,j] = perr
#                             
#                             popt_bi_c = np.zeros(5)
#                             perr_bi_c = np.zeros(5)
#                             if popt0_bi[1] > popt0_bi[3]:
#                                 popt_bi_c[0] = popt0_bi[2]  
#                                 popt_bi_c[1] = popt0_bi[3]  
#                                 popt_bi_c[2] = popt0_bi[0]  
#                                 popt_bi_c[3] = popt0_bi[1]                                  
#                                 popt_bi_c[4] = popt0_bi[4] 
#                                 
#                                 perr_bi_c[0] = perr_bi[2]  
#                                 perr_bi_c[1] = perr_bi[3]  
#                                 perr_bi_c[2] = perr_bi[0]  
#                                 perr_bi_c[3] = perr_bi[1]                                  
#                                 perr_bi_c[4] = perr_bi[4] 
#                             else:
#                                 popt_bi_c[0] = popt0_bi[0]  
#                                 popt_bi_c[1] = popt0_bi[1]  
#                                 popt_bi_c[2] = popt0_bi[2]  
#                                 popt_bi_c[3] = popt0_bi[3]                                  
#                                 popt_bi_c[4] = popt0_bi[4] 
#                                 
#                                 perr_bi_c[0] = perr_bi[0]  
#                                 perr_bi_c[1] = perr_bi[1]  
#                                 perr_bi_c[2] = perr_bi[2]  
#                                 perr_bi_c[3] = perr_bi[3]                                  
#                                 perr_bi_c[4] = perr_bi[4] 
#                             lifetime_biexp_array[i,j] = popt_bi_c
#                             lifetime_biexp_std_array[i,j] = perr_bi_c
#                             
#                             
#                     print("lifetime_array: ", lifetime_array)
#                     print("lifetime_std_array: ", lifetime_std_array)
#                     print("lifetime_array: ", lifetime_biexp_array)
#                     print("lifetime_std_array: ", lifetime_biexp_std_array)
#                     print("std_ratio: ", lifetime_std_array/lifetime_array)
#                     H["lifetime_array"] = lifetime_array
#                     H["lifetime_std_array"] = lifetime_std_array
#                     H["lifetime_biexp_array"] = lifetime_biexp_array
#                     H["lifetime_biexp_std_array"] = lifetime_biexp_std_array
...
#         #print(x,y)
    ...
    def biexponential_function(self, x, a, b, c, d, e):
