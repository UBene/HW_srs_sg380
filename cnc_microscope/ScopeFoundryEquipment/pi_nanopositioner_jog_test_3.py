...
class PINanopositioner(object):
    ...
    def startwave(self):
        ...
        self.pidevice.WOS(('1', '2', '3'), (0.0, 0.0, 0.0))  # Set wavegenerator level offset
        # self.pidevice.WGO(wavegens, mode=[1] * len(wavegens))
        self.pidevice.WGO(wavegens, mode=[257] * len(wavegens))  # code 257 means start at end position
    ...
    def endwave(self):
        wavegens = (1, 2, 3)
        wavetables = (1, 2, 3)
        print('ending wave generators {}'.format(wavegens))
        print('current position is {}'.format(self.pidevice.qPOS(('1', '2', '3'))))
        self.pidevice.WGO(wavegens, mode=[0] * len(wavegens))
        # self.pidevice.STP(noraise=True)
        if self.pidevice.HasWCL():  # you can remove this code block if your controller does not support WCL()
            print('clear wave tables {}'.format(wavetables))
            self.pidevice.WCL(wavetables)
    
    def jogging(self, xt = None, yt = None, zt = None,
                xv = None, yv = None, zv = None):

        '''
        THIS IS NOW DEPRECATED. KEPT FOR DEBUGGING PURPOSE ONLY.
    ...
    def z_comp_wave(self, x_r, y_r, xt, yt):
    ...
    def loadWaveTable(self, xWave, yWave, zWave, shutter, tablerate=100):
        # duration of a wave table point = servo cycle time * tablerate (default servo cycle time = 50us)

        # load WaveTable to controller
        ...
        wavedata = [xWave.tolist(), yWave.tolist(), zWave.tolist()]
        startpos = [wavedata[i][0] for i in range(len(wavedata))]
        # startpos[2] = self.pidevice.qPOS('3')['3']
        print('move axes {} to start positions {}'.format(axes, startpos))
        self.pidevice.MOV(axes, startpos)
        pitools.waitontarget(self.pidevice, axes)
        print('current position is {}'.format(self.pidevice.qPOS(('1', '2', '3'))))

        # connect to wavegenerators
        if self.pidevice.HasWCL():  # you can remove this code block if your controller does not support WCL()
            print('clear wave tables {}'.format(wavetables))
            self.pidevice.WCL(wavetables)  # Clear all the given wavetables

        for i, wavetable in enumerate(wavetables):
            print('write wave points of wave table {} and axis {}'.format(wavetable, axes[i]))
            pitools.writewavepoints(self.pidevice, wavetable, wavedata[i], bunchsize=10)

        # Code to connect shutter to DigitalOut Line of the Controller (to be implemented)
        shutter_edge = np.diff(shutter)
        self.pidevice.CTO(1, 3, 4)
        print('loading shutter commands to controller')
        self.pidevice.TWS(1, 1, 3)
        for count, trigger in enumerate(shutter_edge):
            if trigger == 1:  # set shutter voltage to high
                self.pidevice.TWS(1, count+1, 2)
            elif trigger == -1:  # set shutter voltage to low
                self.pidevice.TWS(1, count+1, 3)
        ...
        if self.pidevice.HasWGC():  # you can remove this code block if your controller does not support WGC()
            ...
            self.pidevice.WGC(wavegens, NUMCYLES)
        ...
        if self.pidevice.HasWTR():  # you can remove this code block if your controller does not support WTR()
            ...
            self.pidevice.WTR(0, [tablerate], 1) # 3rd parameter determines interpolation type (0: no interpolation, 1: linear interpolation)
