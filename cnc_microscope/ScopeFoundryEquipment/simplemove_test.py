...
STAGES = ('P-545.3R8H','P-545.3R8H','P-545.3R8H')  # connect stages to axes
...
def main():
    ...
    with GCSDevice(CONTROLLERNAME) as pidevice:
        ...
        ID = pidevice.EnumerateUSB('E-727')
        #pidevice.InterfaceSetupDlg(key='sample')
        # pidevice.ConnectRS232(comport=1, baudrate=115200)

        print('ID: ', ID)
        pidevice.ConnectUSB(ID[0])
        # pidevice.ConnectRS232(comport=1, baudrate=115200)
        ...
        for _ in range(5):
            ...
            print ("pidevice.axes", pidevice.axes)
