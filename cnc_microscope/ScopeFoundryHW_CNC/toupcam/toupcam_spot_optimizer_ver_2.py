...
from ScopeFoundryHW_CNC.toupcam.toupcam_live_measure_ver_2 import ToupCamLiveMeasure
...
#### Begin of the toupcam optimizer class ####
class ToupCamSpotOptimizer(ToupCamLiveMeasure):
    ...
    def setup(self):
        ...
        ## This is the setup for Spot Optimization Section
    ...
    def update_display(self):
        ...
        # im = np.flip(self.get_rgb_image(), 0)
        # im[..., 1] = -im[..., 1].copy()
        ...
        # Plot Preview Pattern
        if hasattr(self, 'Xt'):
            # Scale self.Xt and self.Yt (To be implemented)
            px_per_fUni = 1/(um_per_px * self.fUni_per_um) # need to adjust dynamically to scope scale
            Xo = self.Xt[0]
            Yo = self.Yt[0]

            self.Xt = self.Xt - Xo
            self.Yt = self.Yt - Yo
            self.Xt = px_per_fUni * self.Xt
            self.Yt = px_per_fUni * self.Yt
            self.Xt = self.Xt + Xo
            self.Yt = self.Yt + Yo

            self.fUni_per_um = 1/um_per_px;

            # Translate self.Xt and self.Yt (first point on (Xo, Yo) from spot)
            self.Xt = self.Xt - Xo + (spot_pos_x + self.Xo)
            self.Yt = self.Yt - Yo + (spot_pos_y + self.Yo)

            # Plot the curve
            self.job_preview = pg.PlotCurveItem(self.Xt, self.Yt)
            if hasattr(self, 'old_job_preview'):
                self.plot.removeItem(self.old_job_preview)
            self.plot.addItem(self.job_preview)
            self.old_job_preview = self.job_preview
