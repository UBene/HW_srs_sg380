...
class DataBrowser(BaseApp):
    ...
    def setup(self):
        ...
        logo_icon = QtGui.QIcon(
            sibling_path(__file__, "foundry_data_bowser_logo_1024.png")
        )
...
class HyperSpectralBaseView(DataBrowserView):
    ...
    def setup(self):
        ...
        self.spec_dock = self.dockarea.addDock(
            name="Spec Plot", widget=self.graph_layout
        )
        ...
        self.rect_plotdata = self.spec_plot.plot(
            y=[0, 2, 1, 3, 2], pen=self.line_colors[0]
        )
        self.point_plotdata = self.spec_plot.plot(
            y=[0, 2, 1, 3, 2], pen=self.line_colors[1]
        )
        ...
        self.corr_plotdata = pg.ScatterPlotItem(
            x=[0, 1, 2, 3, 4],
            y=[0, 2, 1, 3, 2],
            size=17,
            pen=pg.mkPen(None),
        ...
        # Point ROI
        self.circ_roi = pg.CircleROI(
            (0, 0), (2, 2), movable=True, pen=self.line_colors[1]
        )
        ...
        self.x_axis = self.settings.New(
            "x_axis", str, initial="default", choices=self.default_x_axis_choices
        )
        self.x_axis.add_listener(self.on_change_x_axis)

        self.norm_data = self.settings.New("norm_data", bool, initial=False)
        self.bg_subtract = self.settings.New(
            "bg_subtract",
            str,
            initial="None",
            choices=("None", "bg_slice", "costum_const"),
        )
        ...
        self.spatial_binning = self.settings.New(
            "spatial_binning", int, initial=1, vmin=1
        )
        ...
        self.cor_Y_data = self.settings.New(
            "cor_Y_data", str, choices=self.default_display_image_choices, initial="sum"
        )

        # peakutils
        self.peakutils_settings = LQCollection()
        self.show_peak_line = self.peakutils_settings.New(
            "show_peak_line", bool, initial=False
        )
        self.show_peak_line.add_listener(self.update_display)
        self.baseline_deg = self.peakutils_settings.New(
            "baseline_deg", int, initial=0, vmin=-1, vmax=100
        )
        self.baseline_deg.add_listener(self.update_display)
        self.thres = self.peakutils_settings.New(
            "thres", float, initial=0.5, vmin=0, vmax=1
        )
        ...
        self.base_plotdata = self.spec_plot.plot(
            y=[0, 2, 1, 3, 2], pen=self.line_colors[2]
        )
        self.peak_lines = []

        # data slicers
        self.x_slicer = DataSelector(
            plot_data_item=self.base_plotdata, name="x_slice", initials=[100, 511, 1],
        )
        self.bg_slicer = DataSelector(
            plot_data_item=self.base_plotdata, name="bg_slice",
        )
    ...
    def get_bg(self):
        ...
        if bg_subtract_mode == "bg_slice":
            ...
            self.bg_slicer.set_label(
                title=bg_subtract_mode,
    ...
    def get_xhyperspec_data(self, apply_use_x_slice=True):
        ...
        if binning != 1:
            x, hyperspec_data = bin_y_average_x(
                x, hyperspec_data, binning, -1, datapoints_lost_warning=False
            )
            bg *= binning
    ...
    def on_change_data_filename(self, fname):
        ...
        try:
            ...
            if self.settings["spatial_binning"] != 1:
                self.hyperspec_data = bin_2D(
                    self.hyperspec_data, self.settings["spatial_binning"]
                )
                self.display_image = bin_2D(
                    self.display_image, self.settings["spatial_binning"]
                )
    ...
    def reset(self):
        ...
        """
        keys_to_delete = list(
            set(self.display_images.keys()) - set(self.default_display_image_choices)
        )
    ...
    def on_change_rect_roi(self, roi=None):
        ...
        self.on_change_corr_settings()
        self.update_peaks(
            *self.get_xy(roi_slice, apply_use_x_slice=True), pen=self.line_colors[0]
        )
    ...
    def on_update_circ_roi(self, roi=None):
        ...
        self.on_change_corr_settings()
        self.update_peaks(
            *self.get_xy(self.circ_roi_slice, apply_use_x_slice=True),
    ...
    def on_change_corr_settings(self):
        try:
            ...
            self.corr_plotdata.addPoints(
                x=x_circ, y=y_circ, pen=pg.mkPen(self.line_colors[1], width=3)
            )
    ...
    def save_state(self):
        ...
        with h5_file as h5_file:
            ...
            h5_io.h5_save_lqcoll_to_attrs(
                self.x_slicer.settings, h5_group_settings_group
            )
            h5_group_settings_group = h5_file.create_group("bg_slicer_settings")
            h5_io.h5_save_lqcoll_to_attrs(
                self.bg_slicer.settings, h5_group_settings_group
            )
...
def bin_2D(arr, binning=2):
    ...
    arr = (
        arr[:, 0:salvaged_along_dim].reshape((shape[0], -1, binning, *shape[2:])).sum(2)
    )
...
def peak_map(
    hyperspectral_data, wls, thres, min_dist, refinement, ignore_phony_refinements
):
