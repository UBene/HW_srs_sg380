...
from ScopeFoundry.helper_funcs import (QLock, bool2str, get_logger_from_class,
                                       str2bool)
...
class LoggedQuantity(QtCore.QObject):
    ...
    **LoggedQuantity** objects are containers that wrap settings. These settings
    ...
    def read_from_hardware(self, send_signal=True):
        self.log.debug("{}: read_from_hardware send_signal={}".format(
            self.name, send_signal))
        if self.hardware_read_func is not None:
            ...
            self.log.warn(
                "{} read_from_hardware called when not connected to hardware".format(self.name))
        return self.val
        ...
        **Arguments:**  **Description:**
        ...
        with self.lock:
            ...
            if self.same_values(self.oldval, new_val):
                self.log.debug("{}: same_value so returning {} {}".format(
                    self.name, self.oldval, new_val))
                return
            else:
                self.log.debug("{}: different values {} {}".format(
                    self.name, self.oldval, new_val))
        ...
        if (not self.same_values(self.oldval, self.val)) or (force):
            ...
            if self.choices is not None:
                ...
                if self.val in choice_vals:
                    self.updated_choice_index_value.emit(
                        choice_vals.index(self.val))
            self.oldval = self.val
        else:
            # no updates sent
            pass
    ...
    def connect_to_widget(self, widget):
        ...
        **Arguments**  **Description**
        ...
        if type(widget) == QtWidgets.QDoubleSpinBox:
            ...
            self.updated_choice_index_value[int].connect(
    ...
    def connect_to_pushButton(self, pushButton, colors=['rgba( 0, 255, 0, 180)', 'rgba(255, 69, 0, 180)'],
        ...
        pushButton.setStyleSheet(
            pushButton.styleSheet() + s + styleSheet_amendment)
    ...
    def new_pg_parameter(self):
        ...
        if self.choices:
            ...
            p = Parameter.create(name=self.name, type='list', values=dict(
                self.choices), value=self.value)
            ...
            self.updated_value[self.dtype].connect(
                update_param)  # (lambda v, p=p: p.setValue(v))
        ...
        if self.is_array:
            ...
            p = Parameter.create(
                name=self.name, type='str', value=str(self.value))
...
class ArrayLQ(LoggedQuantity):
    ...
    def create_tableView(self, **kwargs):
        ...
        model = ArrayLQ_QTableModel(self, transpose=(
            len(self.val.shape) == 1), **kwargs)
...
class LQCircularNetwork(QtCore.QObject):
    ...
    def __init__(self, lq_dict=None, lq_list=None):
        ...
        # some lock that does NOT allow blocked routines to be executed after release()
        self.locked = False
...
class LQRange(LQCircularNetwork):
    ...
    def __init__(self, min_lq, max_lq, step_lq, num_lq, center_lq=None, span_lq=None, sweep_type_lq=None, space_lq=None, name='lq_range'):
        ...
        lq_dict = {'min': self.min, 'max': self.max,
                   'num': self.num, 'step': self.step}
        ...
        self.step.connect_lq_math(
            (self.min, self.max, self.num), self.calc_step)
...
#        return self._logged_quantities.items()
    ...
    def New_Range(self,
        ...
        LQRange_kwargs = {'min_lq': min_lq, 'max_lq': max_lq,
                          'step_lq': step_lq, 'num_lq': num_lq}
    ...
    def New_Vector(self, name, components='xyz', initial=[1, 0, 0], **kwargs):
        ...
        if len(components) == 3:
            ...
            lq_x = self.New(
                name + '_' + components[0], initial=initial[0], **kwargs)
            lq_y = self.New(
                name + '_' + components[1], initial=initial[1], **kwargs)
            lq_z = self.New(
                name + '_' + components[2], initial=initial[2], **kwargs)
